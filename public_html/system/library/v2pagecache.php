...
class V2PageCache {
    ...
    private $wrapcomment=false; // if set to true (and $addcoment is 
                                // also true), we will wrap comments:
                                // <!--[if IE]><!--comment--><![endif]-->
                                // useful for cloudflare stripping comments

    private $end_flush=false;   // set to true to do an ob_end_flush() before
                                // serving a cached page. Faster, but creates
                                // issues in some setups (default is false)
    ...
    public function __construct() {
        ...
        if (is_array($svar) && array_key_exists('language',$svar)) {
            ...
            // (two letters, optionally followed by - and more letters)
            if (preg_match('/^[a-zA-Z]{2}-*[a-zA-Z]*$/',$svar['language'])) {
                $this->lang=$svar['language'];
            }
        }
        if (is_array($svar) && array_key_exists('currency',$svar)) {
            // only accept 3 consecutive A-Z for $_SESSION['currency']
    ...
    // account also for session, get params, etc of 3rd
    // party themes to force devicetype
    public function GetDevice() {
        if ($this->cachebydevice == "categorizr") {
            $detect=new PC_categorizr();
        } elseif ($this->cachebydevice == "mobiledetect") {
            $detect=new PC_mobiledetect();
        } else {
            $detect=new PC_fakedetect();
        }
        $allowed=array('mobile','tablet','desktop');
        $omtallowed=array('mobile','tablet','desktop','mobile_desktop','tablet_desktop');
        $targetdevice=false;
        if (isset($_GET['change_device'])){
            if (isset($_GET['device_name'])) {
                $targetdevice = $_GET['device_name'];
                if (in_array($targetdevice,$omtallowed)) {
                    $_SESSION['set_device'] = $targetdevice;
                }
            }
        }
        if (isset($_SESSION['set_device'])) {
            $targetdevice=$_SESSION['set_device'];
            if (in_array($targetdevice,$omtallowed)) {
                if($targetdevice=='mobile_desktop' || $targetdevicei =='tablet_desktop'){
                    $device = 'desktop';
                } else {
                    $device = $targetdevice;
                }
            ...
            if (in_array($_GET['view'],$allowed)) {
                $_SESSION['category'] = $targetdevice;
                $targetdevice=$_GET['view'];
            }
        } elseif (isset($_SESSION['category'])) {
            if (in_array($_SESSION['category'],$allowed)) {
                $targetdevice=$_SESSION['category'];
            }
        } elseif (isset($_SESSION['device'])) {
            if (in_array($_SESSION['device'],$allowed)) {
                $targetdevice=$_SESSION['device'];
            }
        } elseif (isset($_COOKIE['device'])) {
            if (in_array($_COOKIE['device'],$allowed)) {
                $targetdevice=$_COOKIE['device'];
            }
        }
        if ($targetdevice === false) {
            if ($detect->isTablet()) {
                $targetdevice="tablet";
        ...
        if (in_array($targetdevice,$allowed)) {
            return $targetdevice;
        } else {
            return 'desktop';
        }
...
// fake class that always returns flase for isMobile() and isTablet() 
class PC_fakedetect {
