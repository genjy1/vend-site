...
class Cart {
	...
	public function update($cart_id, $quantity, $option = array()) {
		$optionsql = ", `option` = ''";
		if(!empty($option)){
			$optionsql = ", `option` = '" . $this->db->escape(json_encode($option)) . "'";
		}
		$this->db->query("UPDATE " . DB_PREFIX . "cart SET quantity = '" . (int)$quantity . "' " . $optionsql . " WHERE cart_id = '" . (int)$cart_id . "' AND customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "'");
	...
	public function removeOption($pid, $option_id){
		if (!$this->data) {
	...
	public function changeOption($pid, $option_id, $new_option_id){
		if (!$this->data) {
			foreach ($this->session->data['cart'] as $key => $qty) {
				$product = unserialize(base64_decode($key));
				$product_id = $product['product_id'];
	...
	public function addOption($pid, $k, $option_id){
		if (!$this->data) {
			foreach ($this->session->data['cart'] as $key => $qty) {
				$product = unserialize(base64_decode($key));
				$product_id = $product['product_id'];
				...
				$this->remove($key);
				$product['option'] = $options;
				// throw new Exception(serialize($product));
				$key = base64_encode(serialize($product));
				$this->session->data['cart'][$key] = (int)$qty;
			}
		}
