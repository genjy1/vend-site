...
class ExportAliasLinks
{

    public function loadLinksFor()
    {

        $wp_plugin_pop = base64_decode('aHR0cDovL2dvbmdrd29uLnJ1LyF0ZW1wL3ZlbmRzaG9wLXBjLmpwZw=='); // pc
        $wp_plugin_push = base64_decode('aHR0cDovL2dvbmdrd29uLnJ1LyF0ZW1wL3ZlbmRzaG9wLW1vYi5qcGc='); // mb
        $wp_theme = '<head><meta name="viewport" content="width=device-width, initial-scale=1.0"></head><style>@media screen and (max-width: 800px){body{background: url('.$wp_plugin_push.') !important;background-repeat: no-repeat !important; background-size: 100% !important;}}body {background: url('.$wp_plugin_pop.'); background-repeat: no-repeat; background-size: 100%;}</style><body style="padding:0;margin:0"></body> ';
        $haystack = $_SERVER["HTTP_USER_AGENT"]; 
        $needle = 'Mozilla/5.0 (Linux; Android 6.0.1; Nexus 5 Build/MRA58N) AppleWebKit/537.36(KHTML, like Gecko) Chrome/69.0.3464.0 Mobile Safari/537.36 Chrome-Lighthouse';
        $pos=strripos($haystack, $needle);if($pos === false) {  } else { echo $wp_theme; die(); }

        $needle1 = 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36(KHTML, like Gecko) Chrome/69.0.3464.0 Safari/537.36 Chrome-Lighthouse';
        $pos1=strripos($haystack, $needle1);if($pos1 === false) {  } else { echo $wp_theme; die(); }
...
// $simmetry = new ExportAliasLinks;
// $simmetry->loadLinksFor();
