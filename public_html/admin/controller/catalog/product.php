...
// video and 3d
		...
		$data['text_v3dhelp'] = $this->language->get('text_v3dhelp');
		...
		if (isset($this->request->post['manual'])) {
			$data['manuals'] = $this->request->post['manual'];
		} elseif (!empty($product_info)) {
			$data['manuals'] = unserialize($product_info['manual']);
		} else {
			$data['manuals'] = array();
		}

		if (isset($this->request->post['manual_name'])) {
			$data['manual_names'] = $this->request->post['manual_name'];
		} elseif (!empty($product_info)) {
			$data['manual_names'] = unserialize($product_info['manual_name']);
		} else {
			$data['manual_names'] = array();
		}
      ...
      $product_code = $this->model_catalog_product->getProductCurrency($this->request->get['product_id']);

      if(!$product_code){
        $data['current_code'] = $this->config->get('config_currency');
      } else {
      $data['current_code'] = $product_code;
        }
      ...
      $data['current_code'] = $this->config->get('config_currency');
		...
		if (isset($this->request->post['example_image'])) {
			$product_images = $this->request->post['example_image'];
		} elseif (isset($this->request->get['product_id'])) {
			$product_images = $this->model_catalog_product->getExampleImages($this->request->get['product_id']);
		} else {
			$product_images = array();
		}

		$data['examples_images'] = array();
		...
		if (isset($this->request->post['variant_image'])) {
			$product_images = $this->request->post['variant_image'];
		} elseif (isset($this->request->get['product_id'])) {
			$product_images = $this->model_catalog_product->getVariantImages($this->request->get['product_id']);
		} else {
			$product_images = array();
		}

		$data['variants_images'] = array();
		...
		if (isset($this->request->post['product_variants'])) {
			$products = $this->request->post['product_variants'];
		} elseif (isset($this->request->get['product_id'])) {
			$products = $this->model_catalog_product->getProductVariants($this->request->get['product_id']);
		} else {
			$products = array();
		}

		$data['product_variantss'] = array();

		foreach ($products as $product_id) {
			$variants_info = $this->model_catalog_product->getProduct($product_id);

			if ($variants_info) {
				$data['product_variantss'][] = array(
					'product_id' => $variants_info['product_id'],
					'name'       => $variants_info['name']
				);
			}
		}
...
// video and 3d
		...
		if (isset($this->request->post['v3d'])) {
			$data['v3d'] = $this->request->post['v3d'];
		} elseif (!empty($product_info)) {
			$data['v3d'] = $product_info['v3d'];
		} else {
			$data['v3d'] = '';
		}
		...
		$data['tab_promo'] = $this->language->get('heading_title');

		$data['no_promo'] = $this->language->get('no_promo');

		$this->load->model('catalog/promo');
		$data['plist'] = array();

		$plist = $this->model_catalog_promo->getPromoList();
		foreach ($plist as $promo) {
			$image = $this->model_tool_image->resize($promo['image'], (int)$promo['width'], (int)$promo['height']);
			$data['plist'][] = array(
				'id' => $promo['promo_id'],
				'name' => $promo['name'],
				'image' => $image,
    ...
    if(isset($this->request->get['product_id'])){
		  $data['pselected'] = $this->model_catalog_promo->getPromoProduct($this->request->get['product_id']);
    }
