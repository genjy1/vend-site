...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		$this->addOrderHistory($order_id, 1, '', false, false, true);
		return $order_id;
	}
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false, $notify_admin = false) {
		...
		if ($order_info) {
			...
			if ($notify_admin && $order_status_id) {
				...
				foreach ($order_product_query->rows as $product) {
                    ...
                    if($this->isInPriceLessCategory($product['product_id'])){
                        $price = "";
                        $hide_total = true;
                        $total = "";
                    } else {
				...
				foreach ($order_total_query->rows as $total) {
					$data['totals'][] = array(
						'title' => $total['title'],
						'text'  => $hide_total == true ? "" : $this->currency->format($total['value'], $order_info['currency_code'], $order_info['currency_value']),
					);
				}
	...
	private function isInPriceLessCategory($product_id)
	{
		$categories = [5];

		$query = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "category WHERE parent_id IN(" . implode(",", $categories) . ")" );
