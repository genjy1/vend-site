        ...
        <div class="grid">
          <?php foreach ($products as $product) { ?>
            ...
            <? } ?>
              ...
              <? } else { ?>
                <div class="price"><? if($avtomat) {?> <span>от </span> <?php echo $product['price']; } ?></div></a></div>
              <? } ?>
          <? } ?>
        </div>
        <?php echo $pagination; ?>
    ...
    <script>
        grecaptcha.ready(function () {
            grecaptcha.execute('6Lcn7DgpAAAAAOtz5NCMN3R4TUUc-JjHYSzKUCJ6', { action: 'contact' }).then(function (token) {
                var recaptchaResponse = document.getElementById('recaptchaResponseCateg');
        ...
        <div class="teldiv"><input type="tel" name="ft" maxlength="2" value = "+7" required >
          ...
          <input type="hidden" name="url" value="<?php echo 'https://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] ?>">
			<input type="hidden" name="recaptcha_response" id="recaptchaResponseCateg">
