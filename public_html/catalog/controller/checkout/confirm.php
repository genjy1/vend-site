...
class ControllerCheckoutConfirm extends Controller {
    ...
    /**
     * Создание заказа (прямая вставка минуя email)
     *
     * @param array $orderData
    ...
    private function createOrder($orderData) {
        try {
            ...
            // Вариант 1: Используем прямую вставку в БД чтобы избежать email timeout
            $orderId = $this->createOrderDirect($orderData);

            if (!$orderId || $orderId <= 0) {
                throw new DatabaseException(
                    'Не удалось создать заказ. Метод вернул невалидный ID.',
                    ''
                );
            }
            ...
            // Пытаемся отправить email асинхронно (не блокируем если не получится)
            $this->sendOrderEmailAsync($orderId);
    ...
    /**
     * Прямое создание заказа в БД (без email)
     *
     * @param array $data
     * @return int
     * @throws Exception
     */
    private function createOrderDirect($data) {
        try {
            ...
            // Вставка истории заказа БЕЗ отправки email
            ...
            $this->logDebug('Order inserted directly into database', [
                'order_id' => $orderId,
            ...
            $this->logError('Error in createOrderDirect', [
                'error' => $e->getMessage(),
                'trace' => $this->formatTrace($e->getTrace())
            ]);
            throw $e;
    ...
    /**
     * Отправка email о заказе асинхронно (не блокирует выполнение)
     *
     * @param int $orderId
     */
    private function sendOrderEmailAsync($orderId) {
        try {
            // Проверяем что есть метод addOrderHistory
            if (!is_object($this->model_checkout_order ?? null)) {
                $this->logWarning('Cannot send order email - model not available');
                return;
            }

            if (!method_exists($this->model_checkout_order, 'addOrder')) {
                $this->logWarning('Cannot send order email - addOrder method not available');
                return;
            }

            // Устанавливаем минимальный timeout для email
            $originalTimeout = null;
            if (is_object($this->config ?? null)) {
                try {
                    $originalTimeout = $this->config->get('config_mail_smtp_timeout');
                    $this->config->set('config_mail_smtp_timeout', 3); // 3 секунды максимум
                } catch (Exception $e) {
                    // Ignore
                }
            }
            ...
            try {
                // Пытаемся отправить email с подавлением ошибок
                @$this->model_checkout_order->addOrder($orderId, 1, '', true);
                
                $this->logDebug('Order email sent successfully', [
                    'order_id' => $orderId
                ]);
            } catch (Exception $e) {
                // Логируем но не падаем
                $this->logWarning('Failed to send order email (non-critical)', [
                    'order_id' => $orderId,
                    'error' => $e->getMessage()
                ]);
            } finally {
                // Восстанавливаем timeout
                if ($originalTimeout !== null && is_object($this->config ?? null)) {
                    try {
                        $this->config->set('config_mail_smtp_timeout', $originalTimeout);
                    } catch (Exception $e) {
                        // Ignore
                    }
                }
            ...
            // Полностью игнорируем ошибки отправки email
            $this->logWarning('Error in sendOrderEmailAsync (ignored)', [
                'order_id' => $orderId,
                'error' => $e->getMessage()
            ]);
        }
