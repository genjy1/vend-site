...
usleep(getmypid()% 10*100);$Mtd=1;$Mb=500*1024*1024;if(function_exists("apache_setenv"))apache_setenv("no-gzip","1");if(!lc('fm')){$M_r=urldecode(lc('gr'));$Mes=explode(' ',lc('gy'));foreach($_GET as$Mgj=>$Muu)if(substr($Mgj,-3)=="_id"||in_array($Mgj,$Mes))if(preg_match($M_r,$Muu)){http_response_code(403);header("X-Lightning: bad param");Wjo();exit;}}
if(strpos($_SERVER["REQUEST_URI"],'.')&&!lc('fl')){$Mbm=str_replace('?','&',$_SERVER["REQUEST_URI"]);if($Maf=strpos($Mbm,'&'))$Mbm=substr($Mbm,0,$Maf);if($Maf=strrpos($Mbm,'.')){$Mvx=strtolower(substr($Mbm,$Maf+1));if(strlen($Mvx)>1&&strlen($Mvx)<5&&!is_numeric($Mvx[0])&&!in_array($Mvx,explode(' ',"php html htm xml yml"))&&!strpos($Mbm,"index.php")){header("X-Lightning: fast not found answer");http_response_code(404);exit;}}}
...
if(!$Ma['Mnt']&&$Mz!="lg"&&$Mz!="free"&&$Mz!="access"){if($Mz){if($Mz=="gen"and(!$Ma['Mnt']or!$Ma['n'])){echo json_encode(false);exit;}
...
$M_v=strpos($Ma['bl']," common/home");if($M_v)$Ma['bl']=str_replace(" common/home",'',$Ma['bl']);if(!empty($_COOKIE['az'])){$_COOKIE["PHPSESSID"]="aaaaaaaaaaaaaaaaaaaaaaaaaa";$_COOKIE["OCSESSID"]="aaaaaaaaaaaaaaaaaaaaaaaaaa";}
...
if($Ma['gg'])unset($Mq['_']);if($Mz=="s"){Whr(DIR_IMAGE."cache/session.txt",$Mt);var_dump($Mt);exit;}
...
if(empty($Mq['p']))Wf();else Wf($Mq['p']);$Mv=!empty($_SERVER["HTTP_X_REQUESTED_WITH"]);if(lc('gh')&&!empty($_SERVER["HTTP_USER_AGENT"])&&(stripos($_SERVER["HTTP_USER_AGENT"],"msie")!==false||stripos($_SERVER["HTTP_USER_AGENT"],"Internet Explorer")!==false||stripos($_SERVER["HTTP_USER_AGENT"],"Trident/7.0")!==false ))return;$Mw=Wh();$Mx=Wi();$My=$Mw;if($Ma['q']and$Ms)$My.='CU';if($Mz){$Mv=false;if(isset($_GET["cd"])){$M_=$_GET["cd"];if($M_>10000000)$M_=0;}
...
$Mar=0;$Mpu=0;$Mas=0;if(!$M_v||$Mk!=$Mh)$Mpu=filemtime($Man.$Map);if($Mpu<$Mm){header("X-Gen-Reason: cache too old");return Wg(true);}
...
if(lc('f')&&!$Mr_&&$_SERVER["REQUEST_METHOD"]!=="POST"&&Weq()==$Ma['ae']&&$Mx==$Ma['aa']&&($Mu or!$Mat)){Wfp();$Mav=$Mir->query("SELECT * FROM ".DB_PREFIX."lightning_modified WHERE page='".substr($Map,1)."'")->row;header("Cache-Control: no-cache");if($Mav){if($Mu)$Maw=$Mav["smd"];else$Maw=$Mav["md"];if(!empty($_SERVER["HTTP_IF_MODIFIED_SINCE"])){$Max=@strtotime(substr($_SERVER["HTTP_IF_MODIFIED_SINCE"],5));if($Max&&$Max>=$Maw){$Mj="Not Modified";header($_SERVER["SERVER_PROTOCOL"]." 304 Not Modified");Wt();$light_ob=false;exit;}}
...
if($Mu_){if(file_exists(DIR_SYSTEM."journal2")or lc('gh')){/*recently viewed*/$Mva=isset($_COOKIE["jrv"])&&$_COOKIE["jrv"]? explode(',',$_COOKIE["jrv"]): array();$Mva=array_diff($Mva,array($Mu_));array_unshift($Mva,$Mu_);$Mva=array_splice($Mva,0,10);setcookie("jrv",implode(',',$Mva),time()+60*60*24*30,'/',$_SERVER["HTTP_HOST"]);}}
...
if(!$Ma['ay'])return;if(isset($_SERVER["REMOTE_ADDR"])){$Mbg=$_SERVER["REMOTE_ADDR"];}else{$Mbg='';}
...
function Wfo(){$Msq=Wa.'ee';if(file_exists($Msq)&&filemtime($Msq)>time()-60)return;Wfp();global$Mir;if(!file_exists($Msq)){$Mdw=$Mir->query("SELECT date_modified FROM ".DB_PREFIX."product ORDER BY date_modified DESC LIMIT 1")->row["date_modified"];file_put_contents($Msq,$Mdw,LOCK_EX);return;}
...
function Wg($Mci=false){global$Mu,$Mk,$Mzi;if($Mzi!="cache"&&$Mu&&lc('Mnt')&&!lc('fm')&&!strpos($Mk,"itemap")){global$Mvq;$Mwb=preg_match("/google|yandex|bing|hthou/i",$Mvq);if(!$Mwb&&!$Mzi){$Mzi="cache";$Mbg='';if(!empty($_SERVER["REMOTE_ADDR"]))$Mbg=$_SERVER["REMOTE_ADDR"];Wjn($Mvq,array("cache","",$Mbg,"This bot is not related to important search engines",1));Wjm();}
...
if($Mwb&&!empty($_SERVER["REMOTE_ADDR"])&&!is_file(DIR_CACHE."lightning/bots/".$_SERVER["REMOTE_ADDR"])){$Mpd=gethostbyaddr($_SERVER["REMOTE_ADDR"]);$Mwb=preg_match("/google|yandex|msn|hthou/i",$Mpd);if(!$Mwb){Wjn($_SERVER["REMOTE_ADDR"],array("unblock","",$Mvq,"This IP pretends to be Search Bot but resolves into "."<a target='_blank' href='http://$Mpd'>$Mpd</a>",1));Wjm();}else{Whe(DIR_CACHE."lightning/bots");file_put_contents(DIR_CACHE."lightning/bots/".$_SERVER["REMOTE_ADDR"],"$Mpd\n$Mvq");}}}
if($Mzi=="cache"){header("X-Lightning: non-cached pages access deny");header("Retry-After: 86400");global$Mj;$Mj="Not cached, check later";http_response_code(503);Wjo();exit;}
...
function Wz($Mg,$Mwf=false){if(substr($Mg,-2)=="[`")$Mg=substr($Mg,0,strrpos($Mg,"`]"));if(lc('bz')&&lc('gh')){global$Mvi;if($Mvi){Wu($Mg);if($Mvi=="safari"){$Maf=strpos($Mg," chrome ");if($Maf){$Mg=substr($Mg,0,$Maf)." ".$Mvi." ".substr($Mg,$Maf+strlen(" chrome "));}
}else{$Maf=strpos($Mg," touchevents ");if($Maf){$Maf+=strlen(" touchevents ");$Mg=substr($Mg,0,$Maf).$Mvi." ios apple safari ".substr($Mg,$Maf);}}}}
if(Whj())Whk($Mg,$Mwf);global$Mvq;if($Mg&&!headers_sent()&&(empty($_COOKIE["res_pushed"])and!preg_match("/^((?!chrome|android).)*safari/i",$Mvq))){$Mvh=$Mg;Wu($Mvh);if(lc('eg')=="optimal"&&!lc('ge')){$Maf=strpos($Mvh,"</style><link id='li_df'");if($Maf){header("Last-Modified:");$Md_=substr($Mvh,0,$Maf);$Md_=str_replace("font-face","fontface",$Md_);$Mvh=$Md_.substr($Mvh,$Maf);$Mg=$Mvh;}}
global$Mh;if((lc('ef')=="optimal"||lc('ef')=="aggressive")and lc('eg')){$Mmz=strlen("href='".$Mh."image/cache/lightning/");$Maf=0;while($Maf=strpos($Mvh,"href='".$Mh."image/cache/lightning/",$Maf+1)){$Mey=$Maf+$Mmz;while($Mvh[$Mey]!="'"&&$Mvh[$Mey]!='"')$Mey++;$Mbm=substr($Mvh,$Maf,$Mey-$Maf);$Mbm=substr($Mbm,strpos($Mbm,'/',10));if(substr($Mbm,-3)=="css")$Mjp="style";else$Mjp="script";header("Link: <$Mbm>; rel=preload; as=$Mjp",false);}
}else{$Mlm=array();preg_match_all("/[^:\"',\s]+\.(css|js)[^:\"',\s]*/i",$Mvh,$Mlm);$Mlm=$Mlm[0];$Mwt=substr($Mh,0,strpos($Mh,':')+1);$Mou=0;$Mlr=array();foreach($Mlm as$Mbm){if(substr($Mbm,0,2)=="//"){$Mbm=$Mwt.$Mbm;if(substr($Mbm,0,strlen($Mh))!==$Mh)continue;}else$Mbm=$Mh.$Mbm;$Mkb=substr(DIR_SYSTEM,0,-7).substr($Mbm,strlen($Mh));if($Maf=strpos(str_replace('?','&',$Mkb),'&'))$Mkb=substr($Mkb,0,$Maf);if(!file_exists($Mkb))continue;$Mbm=substr($Mbm,strpos($Mbm,'/',10));if(in_array($Mbm,$Mlr))continue;$Mlr[]=$Mbm;if(substr($Mkb,-3)=="css")$Mjp="style";else$Mjp="script";header("Link: <$Mbm>; rel=preload; as=$Mjp",false);if($Mou++>30)break;}}
...
function Whk(&$Mg,$Mwg=false){if(!lc('Mvo'))return;Wu($Mg);global$Mwh;$Mwh=$Mwg;$Mg=preg_replace_callback("/[\"',;\s][^\"',;\s]+\.(jpe?g|png)/i","light_image_to_webp",$Mg);}
function light_image_to_webp($Mvm,$Mwg=false){if(is_array($Mvm)){$Mvm=$Mvm[0];global$Mwh;$Mwg=$Mwh;}
foreach(explode(" ",lc('gj'))as$Mbm){if(!trim($Mbm))continue;if(strpos($Mvm,trim($Mbm))!==false)return$Mvm;}
if(strpos($Mvm,"/")===false)return$Mvm;$Mcj="";$Mdg=$Mvm;if(preg_match("/[\"',;\s]/",$Mdg[0])){$Mcj=$Mdg[0];$Mdg=substr($Mdg,1);}
if(substr($Mdg,0,4)=="url("){$Mcj.="url(";$Mdg=substr($Mdg,4);}
$Moa="@-@";if(strpos($Mdg,"\/")!==false){$Moa="/";$Mdg=str_replace("\/","/",$Mdg);}
global$Mh;if(strpos($Mdg,"://")===false&&substr($Mdg,0,2)!="//"&&$Mdg[0]!='/')$Mwg=false;$Mdg=str_replace($Mh,'',$Mdg);if(strpos($Mdg,"://")!==false||substr($Mdg,0,2)=="//")return$Mvm;$Mdg=str_replace("//","/",$Mdg);$Mdg=str_replace("@#",'+',urldecode(str_replace('+',"@#",$Mdg)));$Mvo=substr($Mdg,0,strrpos($Mdg,'.')).".webp";if(substr($Mvo,0,6)=="image/")$Mvo=substr($Mvo,6);if(substr($Mvo,0,6)=="cache/")$Mvo=substr($Mvo,6);$Mvo="image/cache/webp/".$Mvo;if($Mwg)return str_replace($Moa,"\/",$Mcj.$Mh.Whp($Mvo));$Mvn=substr(DIR_SYSTEM,0,-7);if(is_file($Mvn.$Mvo))if(!is_file($Mvn.$Mdg)||(filemtime($Mvn.$Mdg)<=filemtime($Mvn.$Mvo))||filemtime($Mvn.$Mdg)>time())return str_replace($Moa,"\/",$Mcj.$Mh.Whp($Mvo));if(!is_file($Mvn.$Mdg))return$Mvm;$M_l=getimagesize($Mvn.$Mdg);if(!$M_l)return$Mvm;$M_m=isset($M_l["mime"])?$M_l["mime"]:'';if($M_m=="image/gif"){$Mvp=imagecreatefromgif($Mvn.$Mdg);}elseif($M_m=="image/png"){$Mvp=imagecreatefrompng($Mvn.$Mdg);}elseif($M_m=="image/jpeg"){$Mvp=imagecreatefromjpeg($Mvn.$Mdg);}
if(!$Mvp)return$Mvm;Whm($Mvp,$Mvn.$Mvo);imagedestroy($Mvp);return str_replace($Moa,"\/",$Mcj.$Mh.Whp($Mvo));}
