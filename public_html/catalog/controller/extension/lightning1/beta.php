...
if(!empty($Ma['bz']))$Mti=array();$Mfv=@disk_free_space(DIR_CACHE);if($Mfv<10)$Mfv=2*500*1024*1024;$Mip=-1;$Mus=substr(DIR_SYSTEM,0,-7)."vqmod/checked.cache";if(file_exists($Mus)&&filesize($Mus)>500000)@unlink($Mus);function Wiq(){Wdd("TRUNCATE TABLE li_products");$Mxv=reset($GLOBALS['Ma']['by']);$Mxv=$Mxv["language_id"];Wdd();}
function Wbu(){return new light_db(Ws());}
class light_db{public$driver;public function __construct($driver){global$Mwx;$Mwx["beta"]=$GLOBALS['Ma']['ci']*60;$this->driver=$driver;global$Mjj;if(!$Mjj and!defined('Lightning'))define('Lightning',1);}
...
public function Wbx($Mhj,$Mbt){if(!$Mbt)return false;$Miu=array_merge(Wby("FROM "," ",$Mhj),Wby("JOIN "," ",$Mhj));foreach($Miu as$Mis){if($this->Wbw(trim($Mis))>=$Mbt)return false;}
return true;}
...
if($Miu&&$Miu[0]==DB_PREFIX."product"){$Mjn=strtoupper(substr(trim($Mhj),0,6));if($Mjn=="INSERT")return;$Mqt=empty($GLOBALS['Ma']['gc']);$Maf=strripos($Mhj,"WHERE ");if(!$Maf)return;$Mix=substr($Mhj,$Maf+6);if(substr($Mix,-1)==")"&&!strpos($Mix,"("))$Mix=substr($Mix,0,-1);$Mqg=false;$Mqh=strtoupper(substr($Mhj,0,$Maf-1));$Maf=strripos($Mqh,"SET ");if($Maf){$Mqi=trim(substr($Mqh,$Maf+4));if(substr($Mqi,0,1)=="`")$Mqi=substr($Mqi,0,1);if(substr($Mqi,0,8)=="QUANTITY"&&substr_count($Mqi,"=")==1){$Mqg=true;$Mqj=trim(substr($Mqi,strpos($Mqi,"=")+1));if(substr($Mqj,0,1)=="("&&substr($Mqj,-1)==")")$Mqj=substr($Mqj,1,-1);$Mqj=trim(str_replace("QUANTITY","",$Mqj));$Mqk=substr($Mqj,0,1);if($Mqk=="+"||$Mqk=="-")$Mqj=trim(substr($Mqj,1));else$Mqk=false;if(substr($Mqj,0,1)=="'"&&substr($Mqj,-1)=="'")$Mqj=substr($Mqj,1,-1);if(!is_numeric($Mqj))$Mqg=false;}}
...
public function Whs($Mhj){global$db_santa_active;if($db_santa_active)return$this->driver->query($Mhj);static$Mwq;if(!$Mwq)$Mwq="'".date("Y-m-d H").":00:00'";return$this->driver->query(str_replace(array("<= NOW()","< NOW()","> NOW()"),array("<= $Mwq","< $Mwq","> $Mwq"),$Mhj));}
public function query($Mhj){$Mhj=(string)$Mhj;if($Mhj=="SELECT * FROM ".DB_PREFIX."setting WHERE `code` = 'module_timezone'"){static$Mwu;if(!$Mwu)$Mwu=$this->driver->query($Mhj);return$Mwu;}
if(strpos($Mhj,"ET time_zone")){static$Mwv;if($Mwv)return;$Mwv=1;}
if(strpos($Mhj,"customer_id = '0') AND date_added < DATE_SUB(NOW(), INTERVAL 1 HOUR)")){$Mji=DIR_CACHE."lightning/".'gd';if(file_exists($Mji)&&filemtime($Mji)>time()-24*60*60)return true;Waw(DIR_CACHE."lightning/");file_put_contents($Mji,'');$Mhj=str_replace("INTERVAL 1 HOUR","INTERVAL 7 DAY",$Mhj);}
...
return$Mhp;}
$Mpo=false;if(strpos($Mhj,DB_PREFIX."light_")||strpos($Mhj,"bf_tmp_"))$Mpo=true;else if(substr($Mhj,0,12)!="SELECT COUNT"&&!stripos($Mhj,"distinct ")){$Mix=Wcz("WHERE ",'',$Mhj);if($Mix){if(!stripos($Mix,"NOW()")and!stripos($Mix,"limit ")and!stripos($Mix,"order ")and!stripos($Mix,"group ")){$Mee=str_replace(array("`",".","_"),'',Wak('','=',$Mix));if(ctype_alnum($Mee))$Mpo=true;}}}
...
function Wcd(&$Mhp){if(empty($Mhp->row["product_id"]))return;if($Mhp->num_rows>100)return;global$Mfh;foreach($Mhp->rows as$Mik){$Mfh[$Mik["product_id"]]=1;}}
...
function Wcp(){global$Mir;if(!$Mir->query("SHOW tables like '".DB_PREFIX."lightning_mail'")->row){$Mir->query("CREATE TABLE ".DB_PREFIX."lightning_mail(mail_id int(11) AUTO_INCREMENT, mail MEDIUMTEXT, KEY (mail_id))");}
if($Mir->query("SHOW tables like '".DB_PREFIX."lightning_product_to_page'")->row)return;$Mir->query("CREATE TABLE ".DB_PREFIX."lightning_product_to_page(product_id int(11), page varchar(36), KEY (product_id), KEY(page))");$Mir->query("CREATE TABLE ".DB_PREFIX."lightning_modified (page varchar(36), cs bigint(13), md int(11), scs bigint(13), smd int(11), KEY (page))");}
...
$Mdt=file_get_contents($Mbs);if(strpos($Mdt,$Mkb))Wb("index_php",false);}else Wb("index_php",false);require_once"tau.php";$Mdt=Whh();if($Mdt){$Mtq=substr(DIR_SYSTEM,0,-7)."robots.txt";file_put_contents($Mtq,$Mdt);if(file_get_contents($Mtq)==$Mdt)Wb("robots_txt",false);}
...
function Wcv($Map){static$Mkl;if(!$Mkl){$Mkl=phpversion();srand(getmypid());usleep(rand(0,10000));}
...
global$Mbd;if($Mbd)return;$config=Wc_("config");if(!$config)return;$Mbd['Mmv']=true;foreach(array("config_error_display","config_error_log","error_display","error_log")as$Mgj){$Mbd[$Mgj]=$config->get($Mgj);$config->set($Mgj,0);}}
...
function Wdl($Mhj){if(strpos($Mhj,"SELECT",10))return false;$Mis=Wcz("FROM "," ",$Mhj);if(!$Mis)return false;$Mla=Wak("FROM $Mis "," ",$Mhj);if(!$Mla||strlen($Mla>3)||$Mla=="WHERE")return false;$Mcj=Wcz("","WHERE",$Mhj);if(!$Mcj)return false;$Mlb=(int)Wcy(",","",Wcz("LIMIT","",$Mhj));if(!$Mlb)return false;$Mlc=Wa2.'cx';if(Whg($Mlc)&&filemtime($Mlc)>time()-60*60)$Miu=unserialize(file_get_contents($Mlc));else$Miu=array();if(empty($Miu[$Mis])){$Mfi=Wdd("SHOW KEYS FROM $Mis WHERE Key_name = 'PRIMARY'");if(!$Mfi){$Miu[$Mis]=-1;file_put_contents($Mlc,serialize($Miu),LOCK_EX);return false;}
...
$Mmf[$Mgj]=$Mmg;}
function Wds($Mgj,$Mad){global$Mml,$Mkx,$Mkw,$Mxo;if($Mgj=="config_language_id"and($Mml or$Mkx))if($Mad!=$Mxo)$Mkw=2;if($Mad==$Mxo&&$Mkw==2)$Mkw=false;}
function Wci($Mgj,$Mad='x'){if(empty($GLOBALS['Ma']['cz']))return-1;if(is_string($Mad)&&substr($Mad,-27)=="index.php?route=common/home")$Mad=substr($Mad,0,-27);global$Mkw,$Mxo;static$Mfu;if($Mkw){if($Mkw==1&&$Mfu==$_SESSION["language"])$Mkw=false;else return-1;}
...
function Wfw($Mce,$Maf,&$Mbm){global$Mrj;if($Mrj){$Mbn=end($Mrj);if($Mbn[0]==$Mce)array_pop($Mrj);}
global$Mti;if(isset($Mti[$Mce])){global$Mtk;Wci($Mtk,$Mbm);}
...
function in_cache($Mgj=NULL,$Miu="product"){if(empty($GLOBALS['Ma']['cj']))return false;$Mjh=debug_backtrace(false,2);$Mjh=$Mjh[1];$Myj=$Mjh["function"];if(!empty($Mjh["class"]))$Myj=$Mjh["class"].$Mjh["type"].$Myj;if(is_null($Mgj))$Mgj=str_replace(array(",",":"),array(", ",": "),str_replace(str_split("[]{}\""), '', json_encode($Mjh["args"])));$Mgj="$Myj $Mgj";global$Myk,$Mjq;$Myk=Wbd("beta",$Mgj);if(is_numeric($Miu)){global$Mjq;if($Mjq<time()-$Miu)$Myk=false;$Miu=false;}
if($Myk!==false){if(!$Miu or Wc_('db')->db->Wbx("FROM ".DB_PREFIX.implode(" FROM ".DB_PREFIX,explode(' ',trim($Miu))),$Mjq)){$Myk=unserialize($Myk);if($Myk=="li_false")$Myk=false;global$Mam,$Mjd,$Mg_;$Mg_++;if($Mam)$Mjd[]=array('Mhj'=>$Mgj,'cr'=>$Mjq,'Mbt'=>0,'Mjv'=>Wce());return true;}}
