...
class ControllerCommonHeader extends Controller {
	public function index() {
		...
		$this->document->addScript('/catalog/view/javascript/feedback/feedback.js?ver=1.5.0');
		// $this->document->addScript('/catalog/view/javascript/snow/snowfall.js');
    ...
    $data['graphs'] = array();
		...
		if ($this->request->server['HTTPS']) {
			//$origin_server = $server = $this->config->get('config_ssl');
			$origin_server = $server = preg_replace("/(http:\/\/)/", "https://", $this->config->get('config_url'));
			$server = preg_replace("/(https:\/\/)/", "https://" . $subdomain, $server);
		} else {
			$origin_server = $server = $this->config->get('config_url');
			$server = preg_replace("/(http:\/\/)/", "http://" . $subdomain, $server);		
		}
  ...
  $data['browser'] = $browser;
		...
		$data['telephone1'] = $data['telephone'][0];
		$data['telephone2'] = $data['telephone'][1];
		...
		foreach ($categories as $category) {
			if ($category['top']) {
				...
				$children_data[] = array(
					'name'     => 'Все товары',
					'href'     => $this->url->link('product/category', 'path=' . 2, true),
				);
				// немного костылей
...
//					$href = $this->url->link('product/category', 'path=' . $category['category_id'], true);
    ...
    $data['push'] = $this->load->controller('push/push');
