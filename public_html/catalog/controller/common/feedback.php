...
class ControllerCommonFeedback extends Controller {
    ...
    // Timeout для email
    private const EMAIL_TIMEOUT = 2; // 2 секунды максимум
    
    // Опция отключения email (если SMTP не работает вообще)
    private const DISABLE_EMAIL = false; // true = отключить email полностью
    ...
    public function index() {
        ...
        try {
            ...
            // Отправка email (с защитой от timeout) - некритично!
            $emailSent = $this->sendFeedbackEmailSafe($data);

            $duration = round((microtime(true) - $startTime) * 1000, 2);

            // ВАЖНО: Всегда возвращаем success, даже если email не отправился
            // Email - это вторичная функция, основная - сохранение данных
    ...
    private function sendFeedbackEmailSafe($data) {
        try {
            // Проверка - возможно email полностью отключен
            if (self::DISABLE_EMAIL) {
                $this->logDebug('Email sending disabled by configuration');
            ...
            // Проверка существования шаблона с учетом темы
            $templatePath = $this->findTemplatePath($template, $themeName);
            
            if (!$templatePath) {
                $this->logError('Template not found', [
                    'template' => $template,
                    'theme' => $themeName,
                    'tried_paths' => $this->getTriedPaths($template, $themeName)
                ]);
                return false;
            }

            $this->logDebug('Template found', [
                'template' => $template,
                'path' => $templatePath
            ]);
    ...
    private function getThemeName() {
        try {
            // Пробуем разные варианты получения темы
            
            // Вариант 1: theme_default_directory (OpenCart 2.x)
            $theme = $this->config->get('theme_default_directory');
            if ($theme) {
                return $theme;
            }

            // Вариант 2: config_theme (OpenCart 2.x альтернативный)
            $theme = $this->config->get('config_theme');
            if ($theme) {
                return $theme;
            }

            // Вариант 3: theme_..._directory (OpenCart 3.x)
            ...
            $this->logWarning('Failed to get theme name', [
                'error' => $e->getMessage()
            ]);
            return 'default';
        }
    ...
    /**
     * Поиск шаблона с учетом темы
     *
     * @param string $template
     * @param string $themeName
     * @return string|false Путь к шаблону или false
     */
    private function findTemplatePath($template, $themeName) {
        // Список возможных путей (в порядке приоритета)
        $possiblePaths = [
            // OpenCart 2.x стандартный путь
            DIR_TEMPLATE . $themeName . '/template/common/feedback/' . $template . '.tpl',
            
            // OpenCart 2.x альтернативный путь
            DIR_TEMPLATE . 'default/template/common/feedback/' . $template . '.tpl',
            
            // OpenCart 3.x путь
            DIR_TEMPLATE . 'common/feedback/' . $template . '.twig',
            
            // Старый путь (без theme)
            DIR_TEMPLATE . 'common/feedback/' . $template . '.tpl',
            
            // Прямой путь (если DIR_TEMPLATE уже содержит theme)
            DIR_TEMPLATE . 'template/common/feedback/' . $template . '.tpl'
        ];
    ...
    /**
     * Получение списка проверенных путей (для логирования)
     *
     * @param string $template
     * @param string $themeName
     * @return array
     */
    private function getTriedPaths($template, $themeName) {
        return [
            DIR_TEMPLATE . $themeName . '/template/common/feedback/' . $template . '.tpl',
            DIR_TEMPLATE . 'default/template/common/feedback/' . $template . '.tpl',
            DIR_TEMPLATE . 'common/feedback/' . $template . '.twig',
    ...
    /**
     * Запись в лог (прямая запись в файл)
     */
    private function writeLog($level, $message, $context = [], $logFile = self::LOG_FILE) {
        try {
            ...
            $logFilePath = defined('DIR_LOGS') ? DIR_LOGS . $logFile : '/tmp/' . $logFile;
            ...
            // Если запись не удалась, пробуем записать в error.log через встроенный механизм
            if ($result === false && is_object($this->log ?? null) && method_exists($this->log, 'write')) {
                $this->log->write('[FEEDBACK] ' . $logEntry);
            }

        } catch (Exception $e) {
            // Последняя попытка - PHP error_log
            @error_log('Failed to write feedback log: ' . $e->getMessage() . ' | ' . $message);
        }
    }
